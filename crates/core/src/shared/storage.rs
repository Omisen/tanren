...
pub struct NewAnswer<'a> {
    ...
    pub answer: &'a str,
    pub answered_at: DateTime<Utc>,
    /// Quando ripresentare la carta. Lo decide chi pianifica, non l'archivio: qui
    /// arriva gia' calcolato.
...
impl Database {
    ...
    pub async fn record_answer(&self, answer: NewAnswer<'_>) -> Result<Card> {
        ...
        sqlx::query(
            "INSERT INTO answers (id, item_id, exercise_type, answered_at, correct, answer)
             VALUES (?, ?, ?, ?, ?, ?)",
        )
    ...
    pub async fn answers(&self, item_id: &str, exercise_type: &str) -> Result<Vec<AnswerLog>> {
        let answers = sqlx::query_as::<_, AnswerLog>(
            "SELECT id, item_id, exercise_type, answered_at, correct, answer
             FROM answers
...
mod tests {
    ...
    fn risposta(correct: bool, answered_at: DateTime<Utc>) -> NewAnswer<'static> {
        NewAnswer {
            ...
            answered_at,
            next_due: Some(answered_at + TimeDelta::days(1)),
        }
    }
    ...
    async fn sono_dovute_le_carte_mai_studiate_e_quelle_scadute() {
        ...
        db.record_answer(NewAnswer {
            ...
            next_due: Some(now - TimeDelta::days(1)),
        ...
        // Mai pianificata: dovuta subito.
        db.record_answer(NewAnswer {
            item_id: "kana:hiragana:う",
            next_due: None,
            ..risposta(true, now)
        })
        .await
